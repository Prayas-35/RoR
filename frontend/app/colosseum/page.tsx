...
function executeCelestialSpell(
  ...
  if (!spellPower) {
    state.log.push(
      `Celestial ${celestial.name} doesn't know spell ${spellName}`
    );
    return;
  }
  ...
  switch (spellName) {
    ...
    case "tempest":
      ...
      if (damage > 0) {
        defender.stats.health = Math.max(0, defender.stats.health - damage);
        state.log.push(
          `Celestial ${celestial.name} cast ${spellName} for ${Math.round(damage)} damage`
        );
      }
      break;
    ...
    case "stun":
      ...
      state.log.push(
        `Celestial ${celestial.name} cast ${spellName}, slowing the enemy`
      );
      break;
    ...
    case "necrotic_touch":
      ...
      state.log.push(
        `Celestial ${celestial.name} cast ${spellName}, applying a damage over time effect`
      );
      break;
    ...
    case "aether_blast":
      ...
      state.log.push(
        `Celestial ${celestial.name} cast ${spellName}, healing for ${Math.round(healAmount)} HP`
      );
      break;
    ...
    case "shield":
      ...
      state.log.push(
        `Celestial ${celestial.name} cast ${spellName}, granting ${Math.round(shieldAmount)} shield`
      );
      break;

    case "rewind":
      state.log.push(
        `Celestial ${celestial.name} attempted to rewind time (special effect)`
      );
      break;

    default:
      state.log.push(
        `Celestial ${celestial.name} cast unknown spell ${spellName}`
      );
  }
}
...
const BlessingEffect = ({
  ...
  return (
    ...
    >
      <div
        className={`blessing-beam ${position === "left" ? "blessing-beam-left" : "blessing-beam-right"}`}
      ></div>
...
export default function GladiatorBattle() {
  ...
  const startBattle = () => {
    ...
    const playerCelestials = selectedGods.map((god) => ({
      ...
      buffs: god.attributes.reduce(
        (acc, attr) => ({ ...acc, [attr.trait_type]: attr.value }),
        {}
      ),
      spells: god.attributes.reduce(
        (acc, attr) => ({ ...acc, [attr.trait_type]: attr.value }),
        {}
      ),
    }));
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="bg-gray-800/80 backdrop-blur-sm p-2 rounded text-center">
                        ...
                        <span className="block text-base font-bold text-white">
                          {aiOpponent.character.attackValue}
                        </span>
                      </div>
                      <div className="bg-gray-800/80 backdrop-blur-sm p-2 rounded text-center">
                        <Shield className="h-4 w-4 text-blue-500 mx-auto mb-1" />
                        ...
                        <span className="block text-base font-bold text-white">
                          {aiOpponent.character.defenceValue}
                        </span>
                      </div>
                      <div className="bg-gray-800/80 backdrop-blur-sm p-2 rounded text-center">
                        <Zap className="h-4 w-4 text-yellow-500 mx-auto mb-1" />
                        ...
                        <span className="block text-base font-bold text-white">
                          {aiOpponent.character.speedValue}
                        </span>
                      </div>
                    </div>
                ...
                <div className="mb-6">
                  <div className="mb-3">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-white text-xs">
                        {battleState.human.stats.health}/100
                      </span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-white text-xs">
                        {battleState.human.stats.shield}
                      </span>
                    </div>
                    <div className="w-full bg-gray-700/80 rounded-full h-2.5">
                      <motion.div
                  ...
                  <div className="flex items-start">
                    ...
                    {log.includes("AI") && (
                      <div className="w-8 h-8 rounded-full overflow-hidden mr-2 flex-shrink-0">
                        <Image
                          src={
                            aiOpponent.character.imageUrl || "/placeholder.svg"
                          }
                          alt={aiOpponent.character.name}
                          width={32}
                          height={32}
              ...
              <div className="flex items-center">
                <div
                  className={`w-3 h-3 rounded-full mr-2 ${
                    battleState.turn % 2 !== 1 ? "bg-amber-500" : "bg-red-500"
                  }`}
                ></div>
                ...
                <div className="mb-6">
                  <div className="mb-3">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-white text-xs">
                        {battleState.ai.stats.health}/100
                      </span>
                    </div>
                    <div className="w-full bg-gray-700/80 rounded-full h-2.5">
                      <motion.div
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-bold">
                        {battleState.log
                          ...
                          .reduce(
                            (total, log) => total + parseInt(log.split(" ")[2]),
                            0
                          )}
                      </span>
                    </div>
